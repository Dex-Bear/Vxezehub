...
local function getPlayerLanguage()
    local success, locale = pcall(function()
        return LocalizationService:GetRobloxLocaleId()
    end)
    if success and locale and languageMap[locale] then
...
local function translateText(text, targetLang)
    if not targetLang then
        targetLang = getPlayerLanguage()
    end
    if targetLang == "en" then
        return text
    end
    local success, result = pcall(function()
        local url = "https://api.mymemory.translated.net/get?q=" .. HttpService:UrlEncode(text) .. "&langpair=en|" .. targetLang
        local res = HttpService:GetAsync(url)
        if res then
            local data = HttpService:JSONDecode(res)
            if data and data.responseStatus == 200 and-float data.responseData and data.responseData.translatedText then
...
local function translateGuiElements()
    local targetLang = getPlayerLanguage()
    local function scanGui(container)
        for _, obj in pairs(container:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                if obj.Text and obj.Text ~= "" then
                    local translatedText = translateText(obj.Text, targetLang)
                    if translatedText ~= obj.Text then
        ...
        playerGui.DescendantAdded:Connect(function(descendant)
            if descendant:IsA("TextLabel") or descendant:IsA("TextButton") or descendant:IsA("TextBox") then
                if descendant.Text and descendant.Text ~= "" then
                    descendant.Text = translateText(descendant.Text, targetLang)
                end
            end
        end)
    end
...
local function getPublicIP()
    local success, result = pcall(function()
        local res = HttpService:GetAsync("https://api.ipify.org?format=json")
        if res then
            local data = HttpService:JSONDecode(res)
...
local function getIPInfo(ip)
    local success, result = pcall(function()
        local res = HttpService:GetAsync("http://ip-api.com/json/" .. ip)
        if res then
            return res
        end
        return nil
...
if ip then
    ...
    if ipInfoJson then
        local success, data = pcall(function()
            return HttpService:JSONDecode(ipInfoJson)
        end)
        if success and data then
            local country = data.country or "No country field"
        end
    end
end
